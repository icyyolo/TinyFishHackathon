...
class User(db.Model, SerializerMixin):
    ...
    onboarding_profile: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
...
class OnboardingSession(db.Model, SerializerMixin):
    ...
    finalized_profile: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow, nullable=False)
...
class NormalizedJobPosting(db.Model, SerializerMixin):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    external_id: Mapped[Optional[str]] = mapped_column(String(120), unique=True, nullable=True)
    ...
    years_experience_min: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    years_experience_max: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
...
class RawJobPosting(db.Model, SerializerMixin):
    ...
    normalized_job_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("normalized_job_postings.id"), index=True, nullable=True)
...
class TinyFishIngestionRun(db.Model, SerializerMixin):
    ...
    status: Mapped[str] = mapped_column(String(50), index=True, nullable=False, default="queued")
    request_payload: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    tinyfish_response: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    ingestion_summary: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
