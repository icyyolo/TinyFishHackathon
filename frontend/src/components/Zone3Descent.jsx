...
function JobRow({ job, side, onSelect }) {
  ...
  return (
    <div ref={ref} className="grid items-center max-w-4xl mx-auto" style={{ gridTemplateColumns: '1fr 14px 1fr' }}>
      {side === 'left' ? (
        <motion.div initial={{ opacity: 0, x: -50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="flex items-center justify-end">
          <motion.button
      ...
      <motion.div initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.3 }} className="flex items-center justify-center">
        <div className="w-3.5 h-3.5 rounded-full" style={{ background: c.accent, boxShadow: `0 0 12px ${c.accent}80, 0 0 24px ${c.accent}30` }} />
      </motion.div>

      {side === 'right' ? (
        <motion.div initial={{ opacity: 0, x: 50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="flex items-center justify-start">
...
function buildInsightModel(jobs, radar, trends) {
  const bestRoles = jobs.slice(0, 4).map((job) => ({
    title: job.title,
    demand: job.matchType === 'wants' ? 'High' : job.matchType === 'balanced' ? 'Moderate' : 'Emerging',
    fit: job.matchRate,
  }))

  const missingCore = radar?.missing_core_skills || []
  const trending = trends || []
  const skillsToLearn = [
    ...missingCore.map((item) => ({ skill: item.skill, priority: item.recommended_priority === 'high' ? 'Critical' : 'High' })),
    ...trending.slice(0, 3).map((item) => ({ skill: item.skill, priority: item.direction === 'up' ? 'High' : 'Medium' })),
  ].slice(0, 5)
...
export default function Zone3Descent({ jobs = [], radar, trends = [], targetRole, onSelectJob, isPollingLinkedIn = false }) {
  ...
  const groupedSections = [
    ...
    { label: 'Twilight Zone - Balanced Matches', depth: 'mid', jobs: jobs.filter((job) => job.depth === 'mid') },
    { label: 'Deep Water - Stretch Matches', depth: 'deep', jobs: jobs.filter((job) => job.depth === 'deep') },
    { label: 'The Abyss - High Skill Ceiling', depth: 'abyss', jobs: jobs.filter((job) => job.depth === 'abyss') },
  ]
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="overflow-visible">
      <div className="relative overflow-visible" style={{ background: 'linear-gradient(180deg, #ecfeff 0%, #cffafe 25%, #a5f3fc 55%, #67e8f9 85%, #22d3ee 100%)' }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 pt-12 pb-16">
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.6 }} className="text-center mb-10">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-800 tracking-tight mb-2">Your Command Center</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
            <InsightCard title="Best Roles to Target" icon={Target} delay={0.2}>
              <div className="space-y-2.5">
                {insights.bestRoles.length > 0 ? insights.bestRoles.map((role, i) => (
                  <div key={`${role.title}-${i}`} className="flex items-center justify-between bg-white/50 rounded-xl px-3.5 py-2.5">
                    <div>
                  ...
                  <p className="text-sm text-slate-500">No live recommendations came back yet. Try another scan after the backend starts.</p>
                )}
              </div>
            </InsightCard>
            ...
            <InsightCard title="Skills to Learn Next" icon={Rocket} delay={0.35}>
              <div className="space-y-3">
                {insights.skillsToLearn.length > 0 ? insights.skillsToLearn.map((skill, i) => (
                  <div key={`${skill.skill}-${i}`} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${skill.priority === 'Critical' ? 'bg-rose-400' : skill.priority === 'High' ? 'bg-amber-400' : 'bg-emerald-400'}`} />
                      <span className="text-sm text-slate-700">{skill.skill}</span>
                    </div>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${skill.priority === 'Critical' ? 'bg-rose-50 text-rose-500' : skill.priority === 'High' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
                      {skill.priority}
                    </span>
                  </div>
                )) : (
                  <p className="text-sm text-slate-500">Skill gap analysis becomes available after the backend returns a radar result for your role.</p>
                )}
              </div>
            </InsightCard>
          </div>
      ...
      <div className="relative overflow-visible" style={{ background: 'linear-gradient(180deg, #0e7490 0%, #0c4a6e 5%, #082f49 14%, #061b30 26%, #041225 46%, #020a18 66%, #01050e 83%, #000208 100%)' }}>
        <Bubbles />
        ...
        */}
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="relative z-10 text-center pt-16 pb-8 px-6">
          <h2 className="text-2xl md:text-3xl font-extrabold text-white/90 tracking-tight mb-2">The Descent</h2>
          <p className="text-white/30 max-w-sm mx-auto text-sm">Surface jobs are the strongest current matches. The deeper you go, the more stretch and upside you will see.</p>
        </motion.div>

        <div className="relative z-10 pb-32">
          {jobs.length === 0 ? (
            <div className="text-center px-6 py-20">
              <p className="text-white/50 text-sm">No live jobs were returned yet. Start the Flask backend and rescan to pull recommendations from the matching engine.</p>
            </div>
              ...
              <div key={section.depth}>
                ...
                <div className="space-y-6 px-4">
                  {section.jobs.map((job) => {
                    ...
                    globalIndex += 1
                    return <JobRow key={job.id} job={job} side={side} onSelect={onSelectJob} />
                  })}
                </div>
              </div>
            )
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center pt-20 pb-8">
            <p className="text-white/15 text-sm font-medium tracking-widest uppercase">You've reached the bottom of the reef</p>
